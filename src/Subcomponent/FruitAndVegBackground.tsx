...
interface Image {
  id: number;
  FruitAndVegBgimg: string;
  fname4: string;
}

const fruitAndVegBackground = () => {
  const [imagesAboutBg, setImagesAboutBg] = useState<Image[]>([]);
  console.log(imagesAboutBg)
  const getUserData4 = async () => {
    const res4 = await axios.get("https://pratik-agro-backend.vercel.app/getFruitAndvegBgImg", {
      headers: {
  ...
  const backgroundImageStyle = imagesAboutBg.length > 0
    ? { backgroundImage: `url(${imagesAboutBg[0].FruitAndVegBgimg})` }
    : {};
...
export default fruitAndVegBackground;
