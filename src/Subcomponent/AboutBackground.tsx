...
const AboutBackground = () => {
  ...
  const getUserData4 = async () => {
    const res4 = await axios.get("https://pratik-agro-backend.vercel.app/getAboutbackgroundImg", {
      headers: {
  ...
  const backgroundImageStyle = imagesAboutBg.length > 0
    ? { backgroundImage: `url(${imagesAboutBg[0].Backgroundimg})` }
    : {};

  return (
    <div className="aboutbg img-fluid" style={backgroundImageStyle}>
     <div className="text-block3">
                <p className='text-white text-decoration-none'><h1>About us</h1></p>
              </div>
    </div>
  );
};
