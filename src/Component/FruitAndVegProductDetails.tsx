...
import FruitAndVegProductDeatilsBackground from '../Subcomponent/FruitAndVegProductDeatilsBackground'
export type post = {
    id:number,
    fname9:string,
...
export default function FruitAndVegProductDetails() {
 ...
 const getData = async () => {
  const res7 = await fetch(`https://pratik-agro-backend.vercel.app/getFruitAndVegproductimg/${id}`,{
    method: "GET",
    headers: {
  ...
  if (res7.status === 422 || !data3) {
    ...
    console.log(data3);
  setpost(data3.fname9);
  setpostimg(data3.FruitAndVegproductimg)
  setpostDescription(data3.FruitandVegProductDescription)
...
const handleRefresh = () => {
    history("/Fruit_and_veg");
  };
    return (
        <>

            <FruitAndVegProductDeatilsBackground/>
